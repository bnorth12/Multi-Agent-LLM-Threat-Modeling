...
## Object Catalog (Lower-Level Contract View)

In addition to interface transport flows, the lower-level model treats key JSON payloads as
explicit contract objects with owners, producers, and consumers.

1. **FrameworkStateSnapshot**
   - Persisted by Pipeline Manager into State Store after each stage.
   - Retrieved by Pipeline Manager when resuming paused runs.

1. **RunRecord**
   - Persisted by Pipeline Manager for run history and status listing.
   - Queried by UI backend client for historical run selection.

1. **GateDecision / ThreatDecision**
   - Produced by HITL gate actions and serialized by Decision Audit Serializer.
   - Persisted for replay, governance review, and rationale traceability.

1. **RunSummaryDTO / StageStatusDTO / GateDTO**
   - Projected from backend objects for the separated GUI.
   - Consumed by execution monitor and run list views.

This ensures that persisted data contracts are threat-modeled directly, not only inferred
from component names.

## Separable GUI and Backend Relationship

The GUI is modeled as a separable subsystem that consumes backend DTO projections via the
backend API client. Backend orchestration, state persistence, and decision audit are modeled
as the source-of-truth subsystem.

1. GUI receives DTO projections, not raw internal objects.
1. Backend retains canonical persisted objects and versioning responsibility.
1. Projection mapping boundaries are explicit to prevent schema drift and stale object replay.
