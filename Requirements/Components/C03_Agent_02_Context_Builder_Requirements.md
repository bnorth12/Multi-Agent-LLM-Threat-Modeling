...
|C03-A02-003|Upstream Compliance Preservation|Agent 2 SHALL preserve the input-compliance and provenance metadata produced by Agent 1 and SHALL emit a structured validation failure when upstream ICD compliance data is missing, stale, or contradictory.|The context builder must not silently normalize away input-quality issues that require analyst attention or orchestrator gating.|Test|Verified by merge-path tests confirming compliance metadata is retained and malformed upstream data blocks downstream progression.|
