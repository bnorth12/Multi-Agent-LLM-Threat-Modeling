...
1. Retrieval-aligned concept corpus
...
1. RAG retrieval quality checks
...
1. Multi-view synchronized analyst workspace
...
1. Preserve original STIX ids, object types, relationships, and source
	provenance metadata.
1. Normalize relationships into retrieval edges (for example
	technique -> mitigated-by -> control).
1. Support periodic refresh and de-duplication by STIX id and modified date.
1. Maintain mappings from STIX objects to abstraction-level flow segments and
	protocol wrappers.
...
1. Query by protocol wrapper returns mapped threats and enforced mitigations.
1. Query by mitigation control returns associated abstractions and realized flow
	segments.
1. Query by threat returns both abstract impact path and concrete enforcement
	points.
1. Query by ATT&CK technique returns relevant protocol wrappers, impacted
	abstractions, and mapped mitigations (including D3FEND when available).
1. Query by CAPEC or CWE returns aligned threat patterns and candidate control
