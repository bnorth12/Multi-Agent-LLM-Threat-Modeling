...
1. Add failure-injection tests for timeout, provider failure, and worker restart.
1. Define rollout toggles and fallback mode to current in-process execution.
1. Document runbook for support and incident triage using `run_id` and `correlation_id`.

## Acceptance Criteria

1. Browser E2E covers submit, gate pause, gate resume, completion, and failure scenarios.
1. Evidence shows run continuity across reload and worker restart.
1. Rollout can be enabled incrementally by feature flag.
1. Operators can diagnose a failed run from event timeline without reproducing locally.

## Out of Scope

1. Organization-wide platform SRE onboarding.
1. Long-term analytics warehouse ingestion.
