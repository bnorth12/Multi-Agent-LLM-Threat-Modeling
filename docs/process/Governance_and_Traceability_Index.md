...
- Sign-off
   ...
   **Usage**:
   ...
   1. Update sprint number (2026-09)
   1. Sprint Lead completes checklist during planning day
   1. Get team sign-off
   1. Archive in planning/Sprint_2026_09/ folder
...
- Sprint end: Mark all as ✅ or carryover
   ...
   **Usage**:
   ...
   1. Update header (sprint dates)
   1. Add requirement rows from sprint backlog
   1. Add to version control & link from sprint plan
   1. Update throughout sprint
...
- ❌ FAIL (red): PR blocked, cannot merge
   ...
   **How It Works**:
   ...
   1. Developer pushes: `git push origin branch-name`
   1. Developer creates PR on GitHub
   1. GitHub Actions AUTOMATICALLY runs sprint-traceability.yml
   1. Workflow checks commit message for "HITL-012" or similar issue ID
...
- Preparing sprint closeout evidence and governance sign-off
   ...
   1. Workflow runs verification script to check requirement link
   1. Workflow checks for test file in issue description
   1. Result: Green ✅ or Red ❌ shown in PR
   1. Reviewer cannot approve until checks pass
   1. Cannot merge until CI/CD checks pass
...
1. Copy `Sprint_Planning_Checklist_Template.md` → `Sprint_2026_09_Planning_Checklist.md`
1. Copy `Sprint_Traceability_Matrix_Template.md` → `Sprint_2026_09_Traceability_Matrix.md`
1. Follow Sprint Lifecycle section, Phase 1 (Planning)
1. Get team signed off on checklist
...
1. Understand requirement AC? ✅
1. Know test file path? ✅
...
1. [ ] Issue links to requirement
1. [ ] Implementation matches requirement AC
1. [ ] Tests exist and cover AC
1. [ ] All CI/CD checks passing
1. [ ] Evidence present in issue
...
1. Review output; address any RED ❌ items immediately
1. Update team on traceability health in standup
...
1. What worked well?
1. Should we update Definition of Done?
1. Should we update Requirements & Issues Policy?
1. Should we improve automation?
