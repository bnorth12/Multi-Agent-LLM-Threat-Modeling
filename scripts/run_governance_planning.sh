...
if [ -n "$POLICY_PROFILE" ]; then
  "$PYTHON_BIN" scripts/governance_autoflow.py --context planning --sprint "$SPRINT" --policy-profile "$POLICY_PROFILE"
else
  "$PYTHON_BIN" scripts/governance_autoflow.py --context planning --sprint "$SPRINT"
fi
